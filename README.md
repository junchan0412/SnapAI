...
![SnapAI 1.6.30 UI 总览](docs/snapai-ui-overview.svg)

![SnapAI 设置界面](docs/snapai-settings.png)

## 1.6.30 版本重点

- 结果写回协调器已从 app target 移入 `SnapAILogic` 实体源码。
- `SnapAILogic` 真实源码迁移推进到 16 个实体文件,剩余 symlink 降至 60 个。
- 审计修复 gate 已覆盖这 16 个迁移文件,防止后续迭代重新引入 app target 重复编译或 symlink 回退。

详细发布说明见 [SnapAI 1.6.30 Release Notes](docs/RELEASE_NOTES_1.6.30.md),阶段性复盘见 [SnapAI 1.6.30 Iteration Report](docs/ITERATION_REPORT_1.6.30.md)。
...
```bash
SNAPAI_RELEASE=1 ./build.sh --release
SNAPAI_RELEASE=1 scripts/package-release.sh 1.6.30
```
