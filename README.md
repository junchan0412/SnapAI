...
![SnapAI 1.6.7 UI 总览](docs/snapai-ui-overview.svg)

![SnapAI 设置界面](docs/snapai-settings.png)

## 1.6.7 版本重点

- iCloud 配置同步新增 schema version、更新时间、设备标识、revision 与本机脏状态保护,多设备编辑时会避免静默覆盖,并在设置页和权限健康中心显示最近同步状态。
- 历史 SQLite 写入改为显式事务处理,写入、删除、重建索引失败时会 rollback,并把最近错误写入权限健康诊断。
- 更新包 SHA256 改为分块流式计算,避免随着 zip 体积增长而把整个安装包读入内存。
- 图片请求新增 base64 data URL 后体积校验;快捷提问的截图/粘贴图片压缩后会显示可见状态,过大时给出明确提示。
- 补齐上述能力的逻辑测试,覆盖 iCloud payload/冲突策略、HistoryStore 写入失败、流式 SHA256 和图片编码体积估算。
...
详细发布说明见 [SnapAI 1.6.7 Release Notes](docs/RELEASE_NOTES_1.6.7.md),阶段性复盘和全量审查见 [SnapAI 1.6.7 Iteration Report](docs/ITERATION_REPORT_1.6.7.md)。
