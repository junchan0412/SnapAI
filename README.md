...
![SnapAI 1.6.47 UI 总览](docs/snapai-ui-overview.svg)

![SnapAI 设置界面](docs/snapai-settings.png)

## 1.6.47 版本重点

- `TextCaptureDiagnostic` 已从 app target 迁移为 `SnapAILogic` 真实源码。
- 新增取词诊断 method/failure 轻量枚举和 App 桥接,让取词诊断不再公开依赖取词实现枚举。
- `SnapAILogic` 数量基线下调为最多 46 个 symlink、至少 30 个真实源码。

详细发布说明见 [SnapAI 1.6.47 Release Notes](docs/RELEASE_NOTES_1.6.47.md),阶段性复盘见 [SnapAI 1.6.47 Iteration Report](docs/ITERATION_REPORT_1.6.47.md)。剩余迁移路径见 [SnapAILogic 迁移计划](docs/LOGIC_TARGET_MIGRATION_PLAN.md)。
...
```bash
SNAPAI_RELEASE=1 ./build.sh --release
SNAPAI_RELEASE=1 scripts/package-release.sh 1.6.47
```
