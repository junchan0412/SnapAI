...
![SnapAI 1.4.2 UI 总览](docs/snapai-ui-overview.svg)

![SnapAI 设置界面](docs/snapai-settings.png)

## 1.4.2 版本重点

- 修复设置窗口置顶按钮的状态语义:未置顶显示空心图钉,已置顶显示实心图钉,命令面板继续使用动作语义。
- 点击置顶或取消置顶后立即刷新窗口层级、前置设置窗口,并同步主菜单和命令面板状态。
- 修复 AI 路由卡片宽度没有铺满设置页的问题,让“当前使用”和“AI 路由”区域保持一致的横向布局。
- 为置顶按钮补充明确的辅助功能状态值,降低图标语义切换带来的理解成本。
- 继承 1.4.1 的轻量 UI、紧凑设置页、结果面板工具条、历史窗口筛选和权限健康中心优化。

详细发布说明见 [SnapAI 1.4.2 Release Notes](docs/RELEASE_NOTES_1.4.2.md),阶段性复盘和后续路线见 [SnapAI 1.4.2 Iteration Report](docs/ITERATION_REPORT_1.4.2.md)。
