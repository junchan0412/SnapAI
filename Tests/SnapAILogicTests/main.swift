...
func testPermissionDiagnosticsFormatting() {
    ...
    expect(diagnostics.contains("API Keys: 1/2 configured; enabled missing 1"), "includes api key health counts")
    expect(diagnostics.contains("Secret Store:"), "includes local secret store diagnostics")
    ...
    expect(brief.contains("Secret Store:"),
           "brief permission diagnostics includes local secret store status")
...
testUpdateCheckerSHA256ReadsFilesCorrectly()
testReleaseDecodesNotesBody()
...
testProviderDisplayHostShowsOnlyHost()
testAnthropicAPIKeyWhitespaceIsIgnoredForRequests()
testAnthropicProviderReadinessCoversConfigureFailures()
testLegacyAISectionAliasResolvesToModelPage()
testAIClientEffectiveRuntimeParametersAreSanitized()
testAIClientEncodedImagePayloadSizing()
...
testDefaultActionsShipDefaultHotKeys()
testAppSettingsCanRestoreDefaultHotKeys()
testAppSettingsMigrationBackfillsMissingDefaultHotKeys()
testTextReplacementPreparationDelay()
testResultContentRenderModeAvoidsStreamingMarkdownReparse()
testMarkdownPresentationBuildsBlocksAndRejectsStaleRefreshes()
testResultLiveOutputStatesPublishIndependently()
testResultOperationFeedbackAndExportFilenameAreActionable()
testResultCompletionStatePublishesOneDeduplicatedSnapshot()
testResultCompletionLifecycleRunsOnceAndResetsCleanly()
...
testRoutingMetricsStoreCoalescesBackgroundPersistenceAndFlushes()
testAIRouterUsesRoutingMetricsForFallbackOrder()
testPromptPrivacyFallbackEvalCorpusPrefersLocalThenCloudFallback()
testPromptPrivacyFallbackEvalCorpusSkipsUnsafeCloudFallbacks()
...
testPrivacySubmissionPreviewDetectsExpandedSecretFormatsWhenRedactionDisabled()
...
testPrivacyPreparedSubmissionPassthroughPreservesRiskProtection()
testPromptPrivacyEvalCorpusKeepsInjectionInUserPayloadAndRedactsSecrets()
testPromptPrivacyFallbackEvalCorpusProtectsSecretInjectionPayloads()
testTextDiffSummary()
...
testLocalSecretStoreEncryptsProviderKeysAtRest()
...
testCloudSettingsPayloadMetadataAndLegacyDefaults()
testICloudPullDecisionProtectsLocalChangesAndConflicts()
testICloudPullIfNeededEndToEndRecovery()
...
testTypewriterBufferDequeuesOnlyIncrementalChunks()
testResultStreamingLifecycleCoordinatesImmediateAndTypewriterPresentation()
...
testResultCommandFactoryKeepsMenuShortcutsAndVisibleActionsConsistent()
...
testHistoryStoreReportsWriteFailures()
testHistoryStoreMigrationAndIndexTransactions()
testHistoryStoreConnectionRecoveryAndConcurrency()
testSettingsPersistenceRecoveryAndValidation()
testHistorySearchDeduplicatesAndRejectsDeletedRows()
testICloudUploadPreservesChangesOnFailure()
testLocalSecretStoreConcurrentWritesAndRecovery()
testStorageRuntimeSyntheticBenchmark()
testServerSentEventParserPreservesFramingAndUnicode()
testServerSentEventParserBoundsMalformedPayloads()
testAIStreamDecoderDetectsIncompleteAndLimitedResponses()
testAIStreamDecoderPreservesProtocolDeltas()
testStreamingAccumulatorHandlesEveryTagSplit()
testResultStreamingLifecycleSleepsWhileIdle()
testMarkdownPresentationPreservesFenceBoundariesAndPlainText()
testAppSettingsHistoryMutationsDoNotCommitWhenPersistenceFails()
testHistorySearchUsesStoreResultsBeforeFacetFiltering()
testHistorySearchFallsBackToMemoryForCompactMatching()
testHistorySearchIncludesLocalSemanticMatches()
