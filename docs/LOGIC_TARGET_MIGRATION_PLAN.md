...
- 版本:1.6.36
- `SnapAILogic` 真实 Swift 源码:19 个
- `SnapAILogic` 剩余 symlink:57 个
- 发布门禁:`scripts/check-logic-symlinks.sh` 要求 symlink 不得超过 57 个,真实源码不得少于 19 个
...
6. 命令描述器小簇
   - 已完成:`ActionCommand`
   - 后续候选:`DisplayBehaviorCommand`, `ActionTemplateLibrary`, `HistoryContextCommand`, `HistoryExportCommand`, `ModelSwitchCommand`, `RoutingContextCommand`
