...
- 版本:1.6.42
- `SnapAILogic` 真实 Swift 源码:25 个
- `SnapAILogic` 剩余 symlink:51 个
- 发布门禁:`scripts/check-logic-symlinks.sh` 要求 symlink 不得超过 51 个,真实源码不得少于 25 个
...
6. 命令描述器小簇
   - 已完成:`ActionCommand`, `DisplayBehaviorCommand`, `WorkModeCommand`, `SettingsToggleCommand`, `ModelSwitchCommand`, `RoutingContextCommand`, `HistoryExportCommand`
   - 后续候选:`ActionTemplateLibrary`, `HistoryContextCommand`
