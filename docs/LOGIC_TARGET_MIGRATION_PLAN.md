...
- 版本:1.6.41
- `SnapAILogic` 真实 Swift 源码:24 个
- `SnapAILogic` 剩余 symlink:52 个
- 发布门禁:`scripts/check-logic-symlinks.sh` 要求 symlink 不得超过 52 个,真实源码不得少于 24 个
...
6. 命令描述器小簇
   - 已完成:`ActionCommand`, `DisplayBehaviorCommand`, `WorkModeCommand`, `SettingsToggleCommand`, `ModelSwitchCommand`, `RoutingContextCommand`
   - 后续候选:`ActionTemplateLibrary`, `HistoryContextCommand`, `HistoryExportCommand`
