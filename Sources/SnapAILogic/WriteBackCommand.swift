...
public enum WriteBackCommandFactory {
  ...
  public static func undoDescriptor(for input: WriteBackCommandInput?)
    -> WriteBackCommandDescriptor?
  {
  ...
  public static func statusSummary(
    for input: WriteBackCommandInput?,
    fallback: String?
  ) -> String? {
    input?.diagnosticSummary ?? fallback
  }

  public static func undoSubtitle(for operation: TextWriteBackOperation) -> String {
    switch operation {
    case .replace:
      return "恢复替换前的原文"
    case .append:
  ...
  public static func undoKeywords(for operation: TextWriteBackOperation) -> String {
    switch operation {
    case .replace:
