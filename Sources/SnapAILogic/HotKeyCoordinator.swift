...
package final class HotKeyCoordinator {
    package init() {}

    package typealias RegisterHotKey = (_ combo: HotKeyCombo, _ handler: @escaping () -> Void) -> UInt32?
    package typealias UnregisterAll = () -> Void

    package func registerAll(settings: AppSettings,
    ...
    package static func shouldRegister(_ combo: HotKeyCombo) -> Bool {
        !combo.isUnset
    }

    package static func conflictMessage(label: String,
                                existingLabel: String,
                                combo: HotKeyCombo) -> String {
        "\(label) 与 \(existingLabel) 冲突: \(combo.displayString)"
    }

    package static func registrationFailureMessage(label: String,
                                           combo: HotKeyCombo) -> String {
