...
package struct AIRequestRoute: Identifiable, Equatable {
    package var providerID: String
    package var providerName: String
    package var modelName: String
    package var reason: String
    package var isLocalEndpoint: Bool = false

    package var id: String { "\(providerID)::\(modelName)" }

    package var diagnosticProviderName: String {
        AIRequestDiagnosticText.metadata(providerName,
                                         fallback: "未知供应商",
                                         maxLength: 80)
    }

    package var diagnosticModelName: String {
    ...
    package var displayRouteNote: String {
        diagnosticReason
    }

    package var fallbackSwitchNote: String {
        "\(diagnosticProviderName) / \(diagnosticModelName) 失败,正在切换备用模型"
    }
}

package enum AIRequestRouter {
    package static func candidates(settings: AppSettings,
                           action: AIAction,
                           sourceText: String,
                           hasImage: Bool,
    ...
    package static func routingTextLength(sourceText: String,
                                  routingTextCharacterCount: Int?) -> Int {
        max(0, routingTextCharacterCount ?? sourceText.count)
    }

    package static func contextFitStatus(modelName: String,
                                 providerName: String,
    ...
    package static func modelSupportsImageInput(modelName: String,
                                        providerName: String) -> Bool {
        ModelCapabilityRegistry.capability(for: modelName,
                                           providerName: providerName).supportsVision
    }

    package static func modelSupportsReasoning(modelName: String,
                                       providerName: String) -> Bool {
        ModelCapabilityRegistry.capability(for: modelName,
                                           providerName: providerName).supportsReasoning
    }

    package enum ProviderReadiness: Equatable, Hashable {
        case ready
        case disabled
        case missingAPIKey
    ...
    package static func providerRecoverySuggestion(providers: [AIProvider],
    ...
    package static func isProviderRequestReady(_ provider: AIProvider) -> Bool {
        providerReadiness(provider).isReady
    }

    package static func scopedSettings(from settings: AppSettings, route: AIRequestRoute) -> AppSettings? {
