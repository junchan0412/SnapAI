...
package struct TextSelectionSnapshot {
    fileprivate let element: AXUIElement
    package let selectedRange: CFRange
...
package struct PasteboardSnapshot: Equatable {
    package static let defaultLimits = PasteboardSnapshotLimits()

    package var items: [[String: Data]]
    package var isComplete: Bool
    package var reasonCode: String
    package var totalByteCount: Int
    package var itemCount: Int
    package var typeCount: Int
    ...
    package var undoRecoveryMessage: String {
        guard !isComplete else { return "剪贴板可安全恢复" }
        return "当前剪贴板内容过大或格式过多,为避免丢失用户剪贴板,已取消自动撤销。请在目标应用中使用系统撤销,或手动恢复。"
    }

    package static func complete(items: [[String: Data]],
                         totalByteCount: Int,
                         itemCount: Int,
    ...
    package static func incomplete(reasonCode: String,
                           totalByteCount: Int,
                           itemCount: Int,
                           typeCount: Int) -> PasteboardSnapshot {
...
package struct PasteboardSnapshotLimits: Equatable {
    package var maxItemCount: Int = 32
    package var maxTypeCount: Int = 128
    package var maxTotalByteCount: Int = 64 * 1024 * 1024
}

package enum TextCaptureMethod: String, Equatable {
    case accessibility = "accessibility"
    case clipboard = "clipboard"
    case service = "service"
}

package enum TextCaptureFailureReason: String, Equatable {
...
package struct TextCaptureOutcome: Equatable {
    package var text: String?
    package var method: TextCaptureMethod?
    package var accessibilityAttempted: Bool
    package var clipboardAttempted: Bool
    package var failureReason: TextCaptureFailureReason?
    package var pasteboardReasonCode: String?
    package var clipboardWaitAttempts: Int

    package var usableText: String? {
        TextCapture.usableCapturedText(text)
    }

    package init(text: String? = nil,
                 method: TextCaptureMethod? = nil,
                 accessibilityAttempted: Bool,
                 clipboardAttempted: Bool,
                 failureReason: TextCaptureFailureReason? = nil,
                 pasteboardReasonCode: String? = nil,
                 clipboardWaitAttempts: Int) {
        self.text = text
        self.method = method
        self.accessibilityAttempted = accessibilityAttempted
        self.clipboardAttempted = clipboardAttempted
        self.failureReason = failureReason
        self.pasteboardReasonCode = pasteboardReasonCode
        self.clipboardWaitAttempts = clipboardWaitAttempts
    }
}
...
package enum TextCapture {
    ...
    package static let targetActivationWaitMicroseconds: useconds_t = 120_000
    package static let transientMenuDismissWaitMicroseconds: useconds_t = 60_000
    package static let clipboardRetryWaitMicroseconds: useconds_t = 70_000
    package static let clipboardChangePollLimit = 80
    package static let clipboardCopyAttemptLimit = 3
    package static let focusedAXTraversalDepth = 4
    package static let targetAXTraversalDepth = 8
    package static let maxAXTraversalNodes = 500

    /// 当前进程是否已被授予辅助功能权限
    package static func hasAccessibilityPermission(prompt: Bool = false) -> Bool {
        let options = [kAXTrustedCheckOptionPrompt.takeUnretainedValue() as String: prompt] as CFDictionary
        return AXIsProcessTrustedWithOptions(options)
    }

    /// 异步获取选中文字。结果在主线程(MainActor)回调。
    package static func capture(preferAX: Bool,
    ...
    package static func captureDetailed(preferAX: Bool,
                                targetApp: NSRunningApplication? = nil,
    ...
    package static func usableCapturedText(_ text: String?) -> String? {
    ...
    package static func selectedTextViaAX() -> String? {
        captureViaAX()
    }

    package static func recentSelectionSnapshot(matching text: String) -> TextSelectionSnapshot? {
        snapshotQueue.sync {
    ...
    package static func clearRecentSelectionSnapshot() {
    ...
    package static func selectedSubstring(in value: String, range: CFRange) -> String? {
    ...
    package static func isAXUIElementRef(_ value: CFTypeRef) -> Bool {
        CFGetTypeID(value) == AXUIElementGetTypeID()
    }

    package static func isAXValueRef(_ value: CFTypeRef) -> Bool {
    ...
    package static func shouldActivateTargetForCapture(targetPID: pid_t?,
    ...
    package static func shouldRetryAccessibilityAfterTargetActivation(preferAX: Bool,
                                                              capturedText: String?,
                                                              targetPID: pid_t?,
    ...
    package static func shouldDismissTransientMenusBeforeCopy(targetPID: pid_t?,
                                                      frontmostPID: pid_t?,
                                                      currentPID: pid_t = ProcessInfo.processInfo.processIdentifier,
    ...
    @discardableResult
    package static func activateTargetForCapture(_ targetApp: NSRunningApplication?,
                                         currentPID: pid_t = ProcessInfo.processInfo.processIdentifier) -> Bool {
        guard let targetApp,
              shouldActivateTargetForCapture(targetPID: targetApp.processIdentifier,
                                             currentPID: currentPID,
    ...
    package static func sendRightArrow() {
        guard let source = CGEventSource(stateID: .combinedSessionState) else { return }
    ...
    package static func pasteboardSnapshotRejectionReason(itemCount: Int,
                                                  typeCount: Int,
                                                  totalByteCount: Int,
    ...
    package static func snapshotPasteboard(_ pb: NSPasteboard,
    ...
    package static func restorePasteboard(_ pb: NSPasteboard, snapshot: PasteboardSnapshot) {
    ...
    package static func shouldRestorePasteboard(expectedChangeCount: Int,
                                        currentChangeCount: Int) -> Bool {
        currentChangeCount == expectedChangeCount
    }

    @discardableResult
    package static func restorePasteboardIfUnchanged(_ pb: NSPasteboard,
