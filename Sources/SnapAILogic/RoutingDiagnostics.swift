...
package enum AIRequestAttemptStatus: String, Equatable, Hashable {
...
package struct AIRequestAttemptDiagnostic: Equatable {
    package var route: AIRequestRoute
    package var status: AIRequestAttemptStatus
    package var message: String?
    package var elapsedMilliseconds: Int? = nil
    package var outputCharacterCount: Int? = nil
    package var fallbackDecision: AIRequestFallbackDecision? = nil

    package var summaryLine: String {
        summaryLine(includeMessage: true)
    }

    package func summaryLine(includeMessage: Bool) -> String {
    ...
    package static func sanitizedMessage(_ message: String, limit: Int = 180) -> String {
        SensitiveTextSanitizer.sanitizedMessage(message, limit: limit)
    }

    package static func elapsedMilliseconds(since start: Date,
                                    now: Date = Date()) -> Int {
        max(0, Int((now.timeIntervalSince(start) * 1_000).rounded()))
    }

    package static func formattedDuration(milliseconds: Int) -> String {
...
package struct AIRequestFallbackDecision: Equatable {
    ...
    package var shouldTryNext: Bool {
        reason == .willTryNext
    }
    ...
    package static func decide(fallbackEnabled: Bool,
                       hasNextRoute: Bool,
...
package struct AIRequestRecoveryHint: Equatable {
    package var code: String
    package var suggestion: String
}

package struct AIRequestContextDiagnostic: Equatable {
    ...
    package static func make(settings: AppSettings) -> AIRequestContextDiagnostic {
...
package struct AIRequestPayloadDiagnostic: Equatable {
    package var messageCount: Int = 0
    package var textCharacterCount: Int = 0
    package var estimatedTextTokens: Int = 0
    package var imageAttachmentCount: Int = 0
    ...
    package static func make(messages: [ChatMessage],
    ...
    package static func estimatedTextTokens(forCharacterCount characterCount: Int) -> Int {
        let safeCount = max(0, characterCount)
        guard safeCount > 0 else { return 0 }
        return max(1, Int((Double(safeCount) / 4.0).rounded(.up)))
    }

    package func contextFitSummary(for route: AIRequestRoute) -> String {
        Self.contextFitSummary(estimatedTextTokens: estimatedTextTokens,
                               modelName: route.modelName,
                               providerName: route.providerName)
    }

    package func imageFitSummary(for route: AIRequestRoute,
    ...
    package func reasoningFitSummary(for route: AIRequestRoute,
    ...
    package static func contextFitSummary(estimatedTextTokens: Int,
                                  modelName: String,
    ...
    package static func contextFitStatus(estimatedTextTokens: Int,
    ...
    package static func imageFitSummary(hasImage: Bool,
                                modelName: String,
                                providerName: String = "") -> String {
    ...
    package static func reasoningFitSummary(requiresReasoning: Bool,
                                    modelName: String,
                                    providerName: String = "") -> String {
    ...
    package func candidateFitIssueSummary(routes: [AIRequestRoute],
                                  hasImage: Bool,
                                  requiresReasoning: Bool) -> String {
    ...
    package static func candidateFitIssueSummary(routes: [AIRequestRoute],
    ...
    package func candidateHardIssueSummary(routes: [AIRequestRoute],
                                   hasImage: Bool) -> String {
    ...
    package static func candidateHardIssueSummary(routes: [AIRequestRoute],
                                          estimatedTextTokens: Int,
                                          hasImage: Bool) -> String {
        guard !routes.isEmpty else { return "none" }
        var contextOverLimit = 0
...
package struct AIRequestDiagnostics: Equatable {
    package static let preflightSkippedRouteDisplayLimit = 5
    package static let suppressedVisibleRecoverySuggestions = Set(["无需处理", "等待请求开始", "等待当前模型返回"])
    package static let suppressedVisibleRecoveryCodes = Set(["none", "pending", "waiting-current-route", "fallback-will-try-next"])

    package var actionName: String
    package var actionRequiresReasoning: Bool
    package var sourceCharacterCount: Int
    ...
    package var actionPipeline: ActionPipelineDiagnostic
    package var context: AIRequestContextDiagnostic
    package var payload: AIRequestPayloadDiagnostic
    package var submissionPrivacy: PrivacySubmissionDiagnostic?
    package var candidateRoutes: [AIRequestRoute]
    package var candidateUnavailabilitySummary: String
    package var candidateUnavailabilityRecoverySuggestion: String
    package var attempts: [AIRequestAttemptDiagnostic]

    package var summaryText: String {
        summaryText(includeAttemptMessages: true)
    }

    package var briefSummaryText: String {
        summaryText(includeAttemptMessages: false)
    }

    package var recommendedRouteSummary: String {
    ...
    package var recommendedRouteIssueSummary: String {
        guard let route = candidateRoutes.first else { return "none" }
        return routeIssueSummary(for: route)
    }

    package var firstRequestRouteSummary: String {
    ...
    package var firstRequestRouteIssueSummary: String {
        guard let route = firstRequestRoute else { return "none" }
        return routeIssueSummary(for: route)
    }

    package var visibleRouteExplanation: String {
        if candidateCount <= 0 {
    ...
    package var requestRecoverySuggestion: String {
        guard let latest = attempts.last else {
            if candidateCount <= 0 {
                let recovery = candidateUnavailabilityRecoverySuggestion.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    package var requestRecoveryCode: String {
        guard let latest = attempts.last else {
    ...
    package static func visibleErrorRecoverySuggestion(diagnostics: AIRequestDiagnostics?,
    ...
    package static func visibleErrorRecoveryCode(diagnostics: AIRequestDiagnostics?,
                                         errorMessage: String?) -> String? {
        guard let visibleError = errorMessage?.trimmingCharacters(in: .whitespacesAndNewlines),
              !visibleError.isEmpty else {
            return nil
    ...
    package static func recoverySuggestion(forErrorMessage errorMessage: String?) -> String? {
        recoveryHint(forErrorMessage: errorMessage)?.suggestion
    }

    package static func recoveryCode(forErrorMessage errorMessage: String?) -> String? {
        recoveryHint(forErrorMessage: errorMessage)?.code
    }

    package static func recoveryHint(forErrorMessage errorMessage: String?) -> AIRequestRecoveryHint? {
    ...
    package static func noCandidateRouteReasonSummary(providers: [AIProvider]) -> String {
    ...
    package static func noCandidateRouteRecoverySuggestion(providers: [AIProvider]) -> String {
    ...
    package var cloudFallbackReviewSummary: String {
    ...
    package func requiresCloudFallbackConfirmation(from failedRoute: AIRequestRoute,
    ...
    package var healthStatusLine: String {
    ...
    package func preflightSkippedRouteSummary(limit: Int) -> String {
    ...
    package var firstRequestRoute: AIRequestRoute? {
    ...
    package var preflightSkippedRoutes: [AIRequestRoute] {
    ...
    package func routeIssueSummary(for route: AIRequestRoute) -> String {
        payload.candidateFitIssueSummary(routes: [route],
                                         hasImage: hasImage,
                                         requiresReasoning: actionRequiresReasoning)
    }

    package func routeHardIssueSummary(for route: AIRequestRoute) -> String {
        payload.candidateHardIssueSummary(routes: [route],
                                          hasImage: hasImage)
    }

    package func shouldSkipRouteBeforeRequest(_ route: AIRequestRoute,
    ...
    package func routeSkipMessage(for route: AIRequestRoute) -> String {
        "跳过明显不适配路由: \(routeHardIssueSummary(for: route))"
    }

    package func skippedAttemptRecoveryCode(_ attempt: AIRequestAttemptDiagnostic) -> String {
    ...
    package func skippedAttemptRecoverySuggestion(_ attempt: AIRequestAttemptDiagnostic) -> String {
        if Self.isRouteConfigurationSkipMessage(attempt.message) {
    ...
    package static func isRouteConfigurationSkipMessage(_ message: String?) -> Bool {
    ...
    package func routeSkipRecoveryCode(for route: AIRequestRoute) -> String {
    ...
    package func routeSkipRecoverySuggestion(for route: AIRequestRoute) -> String {
    ...
    package func routeSkipSwitchNote(for skippedRoute: AIRequestRoute,
    ...
    package func routeDisplayNote(for route: AIRequestRoute) -> String {
    ...
    package func summaryText(includeAttemptMessages: Bool) -> String {
        var lines = [
    ...
    package mutating func mark(route: AIRequestRoute,
    ...
    package init(actionName: String,
                 actionRequiresReasoning: Bool = false,
                 sourceCharacterCount: Int,
                 hasImage: Bool,
                 fallbackEnabled: Bool,
                 autoRouteEnabled: Bool = false,
                 routingPreference: AIRoutingPreference,
                 candidateCount: Int,
                 actionPipeline: ActionPipelineDiagnostic = .empty,
                 context: AIRequestContextDiagnostic = AIRequestContextDiagnostic(),
                 payload: AIRequestPayloadDiagnostic = AIRequestPayloadDiagnostic(),
                 submissionPrivacy: PrivacySubmissionDiagnostic? = nil,
                 candidateRoutes: [AIRequestRoute] = [],
                 candidateUnavailabilitySummary: String = "not-checked",
                 candidateUnavailabilityRecoverySuggestion: String = "",
                 attempts: [AIRequestAttemptDiagnostic] = []) {
        self.actionName = actionName
        self.actionRequiresReasoning = actionRequiresReasoning
        ...
        self.candidateUnavailabilitySummary = candidateUnavailabilitySummary
        self.candidateUnavailabilityRecoverySuggestion = candidateUnavailabilityRecoverySuggestion
        self.attempts = attempts
    }
}

package enum AIRequestDiagnosticText {
    package static func metadata(_ value: String,
                         fallback: String,
