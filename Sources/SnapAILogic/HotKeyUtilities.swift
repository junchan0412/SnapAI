...
extension HotKeyCombo {
    package static let unset = HotKeyCombo(keyCode: 0, modifiers: 0)
    package static let quickPanelDefault = HotKeyCombo(keyCode: UInt32(kVK_Space), modifiers: UInt32(optionKey))

    package var isUnset: Bool { modifiers == 0 }

    package var nsModifierFlags: NSEvent.ModifierFlags {
    ...
    package var keyEquivalent: String {
        KeyCodeMap.keyEquivalent(for: keyCode)
    }
}

package enum HotKeyConflictDetector {
    package struct Conflict: Equatable {
    ...
    package static func conflict(for combo: HotKeyCombo,
                         actions: [AIAction],
    ...
    package static func conflictDetail(for combo: HotKeyCombo,
                               actions: [AIAction],
                               excludingActionID: String?,
                               quickPanelHotKey: HotKeyCombo?,
    ...
    package static func systemWarning(for combo: HotKeyCombo) -> String? {
        guard !combo.isUnset else { return nil }
...
package enum HotKeyRecorderText {
    package static let recordingTitle = "录制中..."
    package static let unsetTitle = "未设置"
    package static let instructions = "点击录制;按 Esc 取消,Delete 清除。建议使用 ⌥ 或 ⌃⌥ 组合,避开系统保留快捷键。"
    package static let recordingHelp = "按下带修饰键的组合键完成录制;Esc 取消本次录制,Delete 清除快捷键。"
    package static let idleHelp = "点击后录制全局快捷键。Esc 取消,Delete 清除。"

    // 录制态强反馈文案(按钮实时显示,取代静态的「录制中...」占位)
    /// 录制中、尚未按下任何修饰键时的占位文案。
    package static let recordingPlaceholder = "按下组合键…"
    /// 录制态尾部提示,让 Esc 取消 / ⌫ 清除 可被发现。
    package static let recordingHint = "Esc 取消 · ⌫ 清除"
    /// 仅按了普通键(无修饰键)被拒绝时的提示。
    package static let beepTitle = "需含 ⌘/⌥/⌃/⇧"
    /// 成功捕获后短暂显示的前缀。
    package static let successPrefix = "✓"
    /// 清除快捷键后短暂显示的文案。
    package static let clearedTitle = "已清除"

    /// 将当前按下的修饰键转为可读串(顺序与 displayString 一致:⌃⌥⇧⌘),用于录制时实时反馈。
    package static func liveModifierDescription(_ flags: NSEvent.ModifierFlags) -> String {
        var s = ""
    ...
    package static func title(for combo: HotKeyCombo, recording: Bool) -> String {
        if recording { return recordingTitle }
        return combo.isUnset ? unsetTitle : combo.displayString
    }

    package static func help(for combo: HotKeyCombo, recording: Bool) -> String {
...
extension KeyCodeMap {
    package static func keyEquivalent(for keyCode: UInt32) -> String {
        switch Int(keyCode) {
