...
/// AI 协议类型
package enum APIProtocol: String, Codable, CaseIterable, Identifiable {
...
public enum TypewriterSpeed: String, Codable, CaseIterable, Identifiable {
    ...
    /// tick 间隔(秒)
    package var tickInterval: TimeInterval {
        switch self {
        case .off: return 0
...
/// 历史记录内容保存策略。
package enum HistoryContentStorage: String, Codable, CaseIterable, Identifiable {
    case full = "完整保存"
...
package struct WorkModeBehavior: Equatable {
...
public enum WorkModePreset: String, Codable, CaseIterable, Identifiable {
    ...
    package var behavior: WorkModeBehavior {
        switch self {
        case .standard:
...
package struct ContextStatusSummary: Equatable {
    package var profileCount: Int
    package var usableProfileCount: Int
    package var activeProfileName: String
    package var activeContextCharacterCount: Int
    package var globalSystemPromptCharacterCount: Int
    package var effectiveSystemPromptCharacterCount: Int

    package var hasActiveContext: Bool {
        activeContextCharacterCount > 0
    }

    package var activeProfileDisplayName: String {
        activeProfileName.isEmpty ? "无" : activeProfileName
    }

    package var shareableActiveContextState: String {
        hasActiveContext ? "set" : "none"
    }

    package var shareableActiveProfileNameCharacterCount: Int {
        hasActiveContext ? activeProfileName.count : 0
    }

    package static func make(settings: AppSettings) -> ContextStatusSummary {
...
/// 一个可配置的快捷键(键码 + 修饰键)
package struct HotKeyCombo: Codable, Equatable, Hashable {
    package var keyCode: UInt32
    package var modifiers: UInt32   // Carbon 修饰键掩码 (cmdKey/optionKey/...)

    package static let askDefault = HotKeyCombo(keyCode: UInt32(kVK_ANSI_A), modifiers: UInt32(optionKey))
    package static let translateDefault = HotKeyCombo(keyCode: UInt32(kVK_ANSI_T), modifiers: UInt32(optionKey))
    package static let polishDefault = HotKeyCombo(keyCode: UInt32(kVK_ANSI_P), modifiers: UInt32(optionKey))
    package static let summarizeDefault = HotKeyCombo(keyCode: UInt32(kVK_ANSI_S), modifiers: UInt32(optionKey))
    package static let explainCodeDefault = HotKeyCombo(keyCode: UInt32(kVK_ANSI_E), modifiers: UInt32(optionKey))
...
/// 键码 → 名称 的最小映射(用于显示快捷键)
package enum KeyCodeMap {
    package static func name(for keyCode: UInt32) -> String {
        let map: [Int: String] = [
