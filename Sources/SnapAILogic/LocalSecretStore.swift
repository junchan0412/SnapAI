...
package enum LocalSecretStore {
    private static let store = Store()

    package static func apiKey(for providerID: String) -> String {
        store.apiKey(for: providerID)
    }

    @discardableResult
    package static func setAPIKey(_ key: String, for providerID: String) -> Bool {
        store.setAPIKey(key, for: providerID)
    }

    @discardableResult
    package static func delete(providerID: String) -> Bool {
        store.delete(providerID: providerID)
    }

    package static func diagnosticSummary() -> String {
    ...
    package struct Store {
        ...
        package init(directoryURL: URL = Store.defaultDirectoryURL(),
             fileManager: FileManager = .default) {
            self.directoryURL = directoryURL
            self.fileManager = fileManager
        }

        package func apiKey(for providerID: String) -> String {
        ...
        @discardableResult
        package func setAPIKey(_ key: String, for providerID: String) -> Bool {
        ...
        @discardableResult
        package func delete(providerID: String) -> Bool {
            guard let providerKey = normalizedProviderID(providerID) else { return false }
            Self.accessLock.lock()
            defer { Self.accessLock.unlock() }
            do {
        ...
        package func diagnosticSummary() -> String {
            Self.accessLock.lock()
            defer { Self.accessLock.unlock() }
            var parts: [String] = []
        ...
        private func loadMasterKey(createIfMissing: Bool) throws -> SymmetricKey {
            ...
            guard try loadEnvelope().providers.isEmpty else { throw StoreError.missingMasterKey }
            try ensureDirectory()
